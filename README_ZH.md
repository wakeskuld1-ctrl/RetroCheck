...
## 本次变更说明 (2026-02-28)
- **已完成**：新增 ACK 异常清理/覆盖/多次清理场景测试，覆盖非法长度、非法状态、非活跃槽写入、ACK 覆盖、A/B 清理影响与多次清理范围验证。
- **评估结论**：当前清理策略触发后，旧槽位数据（含 ACK）会被清理，ACK 不具备跨清理保留能力。

## 待完善事项
- 若业务要求 ACK 跨清理保留，需引入独立命名空间或延迟清理策略，并补充对应测试。
- 明确 ACK 生命周期与 Hub 对账窗口的配置策略，并补充运维指引。
- 将 ACK 清理策略与 Hub 对账策略联动说明，避免误读当前默认行为。
